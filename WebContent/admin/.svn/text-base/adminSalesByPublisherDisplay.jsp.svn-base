...
<div class="contentbox ui-corner-ll">
	...
	<c:choose>
		<c:when test="${option == 'sum' }">
			<h3>Total sales by the publisher within specified range of dates: <b>$<fmt:formatNumber value="${publisherTotals}" minFractionDigits="2" maxFractionDigits="2" /></b></h3>
		</c:when>
		<c:otherwise>
			<h3>Total sales by the publisher within specified range of dates: <b>$<fmt:formatNumber value="${publisherTotals}" minFractionDigits="2" maxFractionDigits="2" /></b></h3>
			<table border="1" style="width: 100%;" >
				<tr><td>Book&nbsp;Title</td><td>Quantity</td><td>Worth</td><td colspan="2" >Purchased&nbsp;Date</td></tr>
			<c:forEach var="row" items="${publisherBooks.rows}" varStatus="status" >
